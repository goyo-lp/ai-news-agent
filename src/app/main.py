...
async def run_pipeline(options: RunOptions) -> int:
    """Run one full digest invocation. Exit codes: 0 success, 1 one-or-more
...
async def run_propose(options: ProposeOptions) -> int:
    ...
    with track_run_usage(run_id=run_id, dry_run=False) as usage:
        spine_summary = await run_propose_spine(settings, run_id=run_id)
...
async def run_both(run_options: RunOptions, propose_options: ProposeOptions) -> int:
    ...
    print("=== news digest ===")
    run_exit = await run_pipeline(run_options)
    print("=== LinkedIn proposals ===")
    propose_exit = await run_propose(propose_options)
